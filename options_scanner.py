...
class OptionsScanner:
    ...
    def _fetch(self, ticker, max_expirations=3, min_dte=None, max_dte=None):
        ...
        all_exp = list(tk.options)
        if not all_exp:
            raise ValueError(f"No expirations listed for {ticker}")

        # Pick expirations by DATE, not by position. Names like AMZN/TSLA/AMD
        # run near-daily expirations, so the first N are all short-dated and a
        # later DTE filter would throw every one of them away.
        if min_dte is not None or max_dte is not None:
            today = datetime.now(timezone.utc).date()
            lo = min_dte if min_dte is not None else 0
            hi = max_dte if max_dte is not None else 10_000
            in_window = []
            for e in all_exp:
                try:
                ...
                if lo <= d <= hi:
                    in_window.append(e)
            expirations = in_window[:max_expirations]
            if not expirations:
                raise ValueError(
                    f"no expirations between {lo} and {hi} DTE "
                    f"(available: {[e for e in all_exp[:8]]})")
        else:
            expirations = all_exp[:max_expirations]
    ...
    def _num(value, default=0.0):
        """Coerce a chain field to a real number.

        yfinance returns NaN for volume/openInterest/bid on illiquid strikes.
        `value or default` does NOT catch that: NaN is truthy in Python, so it
        passes straight through and int(NaN) raises ValueError.
        """
        try:
            v = float(value)
    ...
    def _enrich(self, df):
        ...
        for _, row in df.iterrows():
            ...
            bid = self._num(row.get("bid"))
            ask = self._num(row.get("ask"))
            mid = (bid + ask) / 2 if (bid > 0 and ask > 0) else self._num(row.get("lastPrice"))
            spread = (ask - bid) if (bid > 0 and ask > 0) else np.nan
            spread_pct = (spread / mid * 100) if (mid > 0 and not np.isnan(spread)) else np.nan

            S = self._num(row["spot"])
            K = self._num(row["strike"])
            iv = self._num(row.get("impliedVolatility"))
            kind = row["kind"]
            ...
            rows.append({
                ...
                "volume": int(self._num(row.get("volume"))),
                "open_interest": int(self._num(row.get("openInterest"))),
            })
...
def best_contracts(tickers, signal_map=None, target_delta=0.40,
    ...
    for t in tickers:
        ...
        try:
            raw = sc._fetch(t, max_expirations=4, min_dte=min_dte, max_dte=max_dte)
            df = sc._enrich(raw)
        except Exception as e:
            print(f"  [options] {t}: {e}")
            continue
        ...
        # no IV -> no greeks, no probability; nothing to say about the contract
        df = df[(df["iv"] > 0) & df["delta"].notna() & df["prob_itm_pct"].notna()]
        if df.empty:
            continue
